...
static char *incl_users, *excl_users,
            *file_path, *syslog_info, *incl_tables;
...
static char incl_table_buffer[2048];
static char incl_table_pattern_buffer[4096];
...
static char syslog_ident_buffer[128]= "mysql_server_auditing";
...
#define DEFAULT_FILENAME_LEN 23
...
static void rotate_log(MYSQL_THD thd, struct st_mysql_sys_var *var,
                       void *var_ptr, const void *save);
static void update_incl_tables(MYSQL_THD thd, struct st_mysql_sys_var *var,
                              void *var_ptr, const void *save);
...
#define EVENT_QUERY 250  // 122 + 128
...
#define EVENT_QUERY_DML_ONLY_SELECT 128

static const char *event_names[]=
{
  "CONNECT", "QUERY", "TABLE", "QUERY_DDL", "QUERY_DML", "QUERY_DCL",
  "QUERY_DML_NO_SELECT", "QUERY_DML_ONLY_SELECT", NULL
};
...
static MYSQL_SYSVAR_SET(events, events, PLUGIN_VAR_RQCMDARG,
       "Specifies the set of events to monitor. Can be CONNECT, QUERY, TABLE,"
           " QUERY_DDL, QUERY_DML, QUERY_DML_NO_SELECT, QUERY_DML_ONLY_SELECT, QUERY_DCL.",
...
static MYSQL_SYSVAR_STR(incl_tables, incl_tables,
       PLUGIN_VAR_RQCMDARG,
       "use ',' split tables that you want to record (only for ONLY_SELECT)", NULL, update_incl_tables, "");
...
struct sa_keyword dml_only_select_keywords[]=
{
  {6, "SELECT", 0, SQLCOM_DML},
  {0, NULL, 0, SQLCOM_DML}
};
                            ...
                            int error_code, const char *type, int take_lock)
{
      ...
      (events & EVENT_QUERY && !cn->log_always))
  {
    ...
    if (events & EVENT_QUERY_DML_ONLY_SELECT)
    {
      if (filter_query_type(query, dml_only_select_keywords)) 
      {

        int status;
        regmatch_t pmatch[1];
        size_t nmatch=1;
        status = regexec(&reg, query, nmatch, pmatch, 0);
        if (0 == status) 
        {
          goto do_log_query;
        }
...
static void update_excl_users(MYSQL_THD thd,
              struct st_mysql_sys_var *var  __attribute__((unused)),
              void *var_ptr  __attribute__((unused)), const void *save)
{
  char *new_users= (*(char **) save) ? *(char **) save : empty_str;
...
static void update_regex(char *incl_tables) 
{
  char * pch;
  pch = strtok(incl_tables,", ");
  //pattern = "(from|join)\\s+.*?\\.?(\\bTABLE1\\b|\\bTABLE2\\b|\\bTABLE3\\b)";
  char pattern[4096] = "(from|join)\\s+.*?\\.?(";
  while (pch != NULL)
  {
    char buff[218];
    sprintf(buff,"\\b%s\\b|",pch);
    strncat(pattern, buff, strlen(buff));
    pch = strtok (NULL, ", ");
  }

  int len = strlen(pattern); 
  pattern[len-1]=')'; //eat last '|'
  pattern[len] = 0;
  memcpy(incl_table_pattern_buffer, pattern, len);
}


static void update_incl_tables(MYSQL_THD thd,
              struct st_mysql_sys_var *var  __attribute__((unused)),
              void *var_ptr  __attribute__((unused)), const void *save)
{
  char *new_tables= (*(char **) save) ? *(char **) save : empty_str;
  size_t new_len= strlen(new_tables) + 1;
  if (!maria_55_started || !debug_server_started)
    flogger_mutex_lock(&lock_operations);
  mark_always_logged(thd);

  if (new_len > sizeof(incl_table_buffer))
    new_len= sizeof(incl_table_buffer);

  memcpy(incl_table_buffer, new_tables, new_len - 1);
  incl_table_buffer[new_len - 1]= 0;

  incl_tables = incl_table_buffer;
  char regex_buf[2048];
  memcpy(regex_buf, incl_table_buffer, new_len - 1); 
  update_regex(regex_buf);

  if (0 != reg.__REPB_PREFIX(allocated)) // if change incl_tables vars. need free first
  {
    regfree(&reg);
  } 
  regcomp(&reg, incl_table_pattern_buffer, REG_EXTENDED | REG_ICASE | REG_NEWLINE);

  fprintf(stderr, "incl_table_pattern_buffer: %s\n", incl_table_pattern_buffer);
  error_header();
  fprintf(stderr, "server_audit_incl_tables set to '%s'.\n", incl_tables);
  if (!maria_55_started || !debug_server_started)
    flogger_mutex_unlock(&lock_operations);
}
